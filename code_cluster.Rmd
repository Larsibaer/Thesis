...
```
```{r check}
data <- na.omit(select(data, case_type, account, created_by, business_service, assignment_group, auto_close, time_worked, reassignment_count, impact, priority, urgency, openedToClosed, created_by_cleaned, sla_has_breached, number))
```

```{r scaling}
data <- cbind(data[15], data[1:6], data[9:11], data[,c(13:14)], scale(data[,c(7,8,12)]))
```
...
data <- data %>% group_by(created_by) %>% filter(n() > 100)
```
...
# One-hot encoding
encoded_df_withCaseNumber <- one_hot_encode(data, c("case_type", "account", "created_by", "business_service", "assignment_group", "auto_close", "impact", "priority", "urgency", "created_by_cleaned", "sla_has_breached"))
