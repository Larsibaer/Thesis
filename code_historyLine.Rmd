...
```{r data_processing}
# Filter out records where update_time is after closed
data <- data %>%
  filter(update_time <= closed)
...
calculate_time_diff <- function(data, field) {
  data %>%
    filter(field == !!field) %>%
    filter(!is.na(new)) %>%
    group_by(set) %>%
