...
# We restart with the salaries dataset 
data(Salaries, package="carData")
str(Salaries)
head(Salaries)
summary(Salaries)
# ------------------------------ Grouping -------------------------- #
# Let's plot salaries and the years of service
ggplot(data, 
       aes(x = data$time_worked, 
           y = data$)) +
  geom_point() + 
  labs(title = "Academic salary by years of service")



# In the next step, let's try to add further information to the scatter plot
ggplot(Salaries, aes(x = yrs.service, 
                     y = salary, 
                     color = sex)) +
  geom_point() +
  labs(title = "Academic salary by sex and years of service") 
...
ggplot(Salaries, 
       aes(x = yrs.service, 
            y = salary, 
            color = rank,
            shape = sex)) +     # adding sex 
  geom_point(size = 3, 
             alpha = .6) +
  labs(title = "Academic salary by rank, sex, and years since degree")


# Only bubble plot - colors with viridis  
ggplot(Salaries, 
       aes(x = yrs.service, 
           y = salary, 
           color = rank,
           size = yrs.since.phd)) + # adding continuous variable (yrs. since phd)
  geom_point(alpha = .5) + 
  scale_color_viridis(discrete = TRUE, option = "D") +
  labs(title = "Academic salary by rank and years of service",
       y = "Salary in US dollars", x = "Years of Service")


# Only bubble plot - colors RColorBrewer, Dark2
display.brewer.all()
