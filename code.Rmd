...
data$created_by_cleaned <- as.factor(ifelse(data$created_by == "admin", "System", ifelse(grepl("@unico.ch", data$created_by), "Unico", "User")))
```
...
ggplot(data %>% filter(time_worked < 400000) %>% filter(reassignment_count < 5) %>% filter(business_percentage < 100) %>% filter(time_worked > 0), 
       aes(x = time_worked, 
           y = openedToClosed, 
           color = impact,
           size = business_percentage)) + # adding continuous variable (yrs. since phd)
  geom_point(alpha = .5) + 
