...
data <- na.omit(select(data, number, account, created_by_group, business_service, assignment_group, impact, priority, urgency, sla_has_breached, case_cause, resolution_code, opened, closed, time_worked, openedToClosed, business_percentage))
```
...
data <- merge(data, cases_data[c('number','description', 'short_description', 'cause', 'close_notes')], by = "number", all.x = FALSE)
```
