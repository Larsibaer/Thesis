...
ggplot(data_all, aes(x = openedToClosed, fill = as.factor(cluster == 3))) +
  geom_histogram(alpha = 0.5, position = "identity", binwidth = 20) +
  scale_fill_manual(values = c("red", "blue"), labels = c("Other Clusters", "Cluster 3")) +
...
cluster_3_services <- data_all %>% filter(cluster == 3) %>% count(business_service)
other_clusters_services <- data_all %>% filter(cluster != 3) %>% count(business_service)

# Plotting
ggplot() +
  geom_bar(data = cluster_3_services, aes(x = business_service, y = n, fill = "Cluster 3"), stat = "identity", position = "dodge") +
...
data_long <- pivot_longer(data, cols = starts_with("word"), names_to = "word", values_to = "tf_idf")

# Calculate the average TF-IDF for each word in each group
word_group_avgs <- data_long %>%
  group_by(group, word) %>%
  summarise(avg_tf_idf = mean(tf_idf, na.rm = TRUE)) %>%
  ungroup()

# Identify the top words for each group
top_words_by_group <- word_group_avgs %>%
  group_by(group) %>%
  top_n(1, avg_tf_idf)  # You can change the number here based on how many top words you want
