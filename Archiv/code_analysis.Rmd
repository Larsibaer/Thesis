...
topic_model_data <- read_csv("Data/topicModel_allText.csv", locale = locale(encoding = "UTF-8"))
...
state_index <- grep("prop_state", colnames(data_gpt))
group_index <- grep("prop_group", colnames(data_gpt))
topic_index <- grep("topic_", colnames(data_gpt))

#Data Selection
data_gpt <- data_gpt %>% select(cluster, created_by_group, account, openedToClosed, topic_index, group_index, state_index, case_cause, resolution_code, impact, priority, urgency)
...
ggplot(data_all, aes(x = openedToClosed, fill = as.factor(cluster == c(4,6)))) +
